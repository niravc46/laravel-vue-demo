                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                >
                                    ...
                                    <div
                                        v-if="
                                            update_property_form_data.type
                                                .id === 'residential'
                                        "
                                        class="row mb-4"
                                        :class="{
...
const { getPropertyDetail } = propertyStore;
let update_property_form_data;
let pdf_file;
({ update_property_form_data } = storeToRefs(propertyStore));
({ pdf_file } = storeToRefs(propertyStore));
const { amenities } = storeToRefs(propertyStore);
const { getAminityList } = propertyStore;
let is_pdf_preview = ref(true);
...
const emit = defineEmits(["update-property-element"]);

let update_property_form_data_rules = {
    name: {
        required: helpers.withMessage("Please enter a property name", required),
    ...
    type: {
        required: helpers.withMessage("Please select property type", required),
    },
    size: {
        required: requiredIf(function () {
            return update_property_form_data.value.type.id === "residential";
        }),
    },
    amenity_id: {
    ...
    address: {
        required: helpers.withMessage("Please enter address details", required),
        minLength: helpers.withMessage(
...
const v$ = useVuelidate(update_property_form_data_rules, update_property_form_data.value);

// broucher upload
let onChangeBrochure = (e) => {
    let file = e.target.files[0];
    update_property_form_data.value.brochure = file;
    is_pdf_preview.value = false;
};
...
getPropertyDetail(route.params.id, axiosConfig);
getAminityList(axiosConfig);

watch(
    () => update_property_form_data.value.type,
    (value) => {
        if (value.id === "commercial") {
            update_property_form_data.value.size = [];
        }
    }
);
//update Property
let updateProperty = async () => {
    ...
    if (is_valid) {
        propertyStore.updateProperty(
            route.params.id,
            update_property_form_data.value,
            axiosConfig
        );
        setTimeout(() => {
